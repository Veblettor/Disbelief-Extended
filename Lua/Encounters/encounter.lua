...
local function RegisterCutscenes()
	...
	local p5endupdate = function(self)
		if self.Phase == 0 then
			...
			if self.ElapsedDelta >= 27.6 and not self:GetVar("hitf") then
				self:SetVar("hitf",true)
				Audio.PlaySound("hitsound")
				PapsRig:SetVisible(false)
			end
			...
			local wingdings = self:GetVar("wingdings")
				...
				if self.ElapsedDelta >= 0.1 and self:GetVar("g2") == 0 then
					...
					g.absx = 160
					table.insert(wingdings,g)
				elseif self.ElapsedDelta >= 0.2 and self:GetVar("g2") == 1 then
					self:SetVar("g2",2)
					Audio.PlaySound("ding")
					local g = CreateSprite("WingDings/point_left","OverOverTop")
					g.y = 200
					g.absx = 200
					table.insert(wingdings,g)
				elseif self.ElapsedDelta >= 0.3 and self:GetVar("g2") == 2 then
					self:SetVar("g2",3)
					Audio.PlaySound("ding")
					...
					g.absx = 240
					table.insert(wingdings,g)
				elseif self.ElapsedDelta >= 0.4 and self:GetVar("g2") == 3 then
					self:SetVar("g2",4)
					Audio.PlaySound("ding")
					...
					g.absx = 280
					table.insert(wingdings,g)
				elseif self.ElapsedDelta >= 0.5 and self:GetVar("g2") == 4 then
					self:SetVar("g2",5)
					Audio.PlaySound("ding")
					...
					g.absx = 320
					table.insert(wingdings,g)
				elseif self.ElapsedDelta >= 0.6 and self:GetVar("g2") == 5 then
					self:SetVar("g2",6)
					Audio.PlaySound("ding")
					...
					g.absx = 360
					table.insert(wingdings,g)
				elseif self.ElapsedDelta >= 0.7 and self:GetVar("g2") == 6 then
					self:SetVar("g2",7)
					Audio.PlaySound("ding")
				...
				if self.ElapsedDelta >= 1.1 and not self:GetVar("staticf") then
...
function EncounterStarting()
	...
	if GetAlMightyGlobal("papyrusdisbeliefex_checkpoint") == 1 then
		...
		enemies[1]["def"] = 20
		enemies[1]["check"] = "[w:4]Reluctant to finish the job.[w:4]\nDon't give him the chance."
		enemies[1]["comments"] = {"No more puzzles.", "No more games.", "Papyrus is reluctant to end you.", "Papyrus is doubting himself."}
...
function Update()
	...
	if GetCurrentState() ~= "DEFENDING" and PapsRig.Legs.alpha ~= 1 and phase == 6 then
		PapsRig:SetAlpha(PapsRig.Legs.alpha + Time.mult/60)
	end
...
function EnteringState(newstate, oldstate)
	...
	if newstate == "ATTACKING" and phase == 4 and enemies[1].GetVar("scared") < 6 then
